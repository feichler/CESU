...
final class Linker
{
    ...
    public function getListAddLink()
    {
        ...
        if ($this->getCrud()->hasParent()) {
            ...
            // last part must be the view action -> discard this
            array_pop($parts);

            if (count($parts) == 1) {
                // directly in a root definition embedded
                $routeName = $parts[0] . '.' . $this->getCrud()->getDefinition()->getRouteSingular() . '.add';
                $params    = array('id' => $this->getCrud()->getParentEntity()->getId());
                $link      = $this->getCrud()->getNavigator()->getLinkFromRoute($routeName, $params);
            }
        } else {
    ...
    public function getListViewLink($entity)
    {
        ...
        if ($this->getCrud()->hasParent()) {
            ...
            if (count($parts) == 1) {
                // directly in a root definition embedded
                $routeName = $parts[0] . '.' . $this->getCrud()->getDefinition()->getRouteSingular() . '.view';
    ...
    public function getListEditLink($entity)
    {
        ...
        if ($this->getCrud()->hasParent()) {
            ...
            if (count($parts) == 1) {
                // directly in a root definition embedded
                $routeName = $parts[0] . '.' . $this->getCrud()->getDefinition()->getRouteSingular() . '.edit';
                $params    = array('id' => $entity->getId());
                $link      = $this->getCrud()->getNavigator()->getLinkFromRoute($routeName, $params);
            }
        } else {
    ...
    public function getListDeleteLink($entity)
    {
        ...
        if ($this->getCrud()->hasParent()) {
            ...
            if (count($parts) == 1) {
                // directly in a root definition embedded
                $routeName = $parts[0] . '.' . $this->getCrud()->getDefinition()->getRouteSingular() . '.delete';
                $params    = array('id' => $entity->getId());
                $link      = $this->getCrud()->getNavigator()->getLinkFromRoute($routeName, $params);
            }
        } else {
    ...
    public function getRedirectAfterProcess()
    {
